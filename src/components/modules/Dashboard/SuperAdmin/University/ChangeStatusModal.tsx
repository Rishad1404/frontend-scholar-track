// src/app/(dashboardLayout)/(superAdminRoutes)/super-admin/universities-management/_components/ChangeStatusModal.tsx
...
import {
  ...
  VALID_UNIVERSITY_STATUS_TRANSITIONS,
...
import { changeUniversityStatusAction } from "@/app/(dashboardLayout)/(superAdminRoutes)/super-admin/universities-management/_actions";
...
export default function ChangeStatusModal({
  ...
  const { mutateAsync, isPending } = useMutation({
    mutationFn: (payload: { status: string }) =>
      changeUniversityStatusAction(university!.id, payload),
  });

  const form = useForm({
    defaultValues: {
      status: "" as UniversityStatus,
    },
  ...
  const validTargets =
    VALID_UNIVERSITY_STATUS_TRANSITIONS[university.status] || [];
  const currentColor =
    UNIVERSITY_STATUS_COLORS[university.status] ?? "#6b7280";
  ...
  return (
    <Dialog open={open} onOpenChange={handleOpenChange}>
      <DialogContent className="sm:max-w-125 p-0 overflow-hidden rounded-[2rem] border-border/40 bg-background/95 backdrop-blur-xl shadow-2xl">
        <VisuallyHidden>
          <DialogTitle>Change Status for {university.name}</DialogTitle>
        </VisuallyHidden>
                    ...
                    {(field) => (
                      <div className="flex flex-col items-center text-center">
                        ...
                        <Select
                          ...
                          onValueChange={(v) =>
                            field.handleChange(v as UniversityStatus)
                          }
                        >
